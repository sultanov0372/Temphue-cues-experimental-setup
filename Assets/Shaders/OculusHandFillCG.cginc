...
half4 applyGlow(int glowType, float3 color, float alpha, float2 texCoord, float3 worldPosition) {
  if (glowType == 30 || glowType == 32) {
    ...
    half4 baseFragment(VertexOutput i) : SV_Target
    {
        ...
        // Add wrist fade logic
        float wristFade = saturate(i.uv.y / _WristFade); // Assumes i.uv.y == 0 near wrist, 1 at fingers
        float finalAlpha = lerp(1.0, i.glowColor.a, wristFade); // Make wrist fully opaque (alpha = 1)
        color.a = finalAlpha;

        if (_GenerateGlow == 1)
        {
            return applyGlow(_GlowType, color.rgb, finalAlpha, i.texcoord1, i.worldPos);
        }
        else
        {
            return half4(color.rgb, finalAlpha);
        }
    }
